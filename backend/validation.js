...
/**
 * Canonical customer identity: 10 digits, no prefix. +91 98765 43210, 919876543210,
 * 09876543210 and 00919876543210 all become 9876543210. Returns null when not a valid Indian mobile.
 */
...
function parseBill(input) {
  ...
  if (typeof input.phone !== 'string') return { error: 'Enter a valid 10-digit customer WhatsApp number.' };
  const phone = input.phone.trim() ? normalizeIndianMobile(input.phone) : '';
  if (phone === null) return { error: 'Enter a valid 10-digit customer WhatsApp number.' };
  ...
  const bill = {
    billId: incomingId, reference, billedAt, branch: input.branch, customer, phone,
    method: input.method, fulfilment, items, discount: input.discount, received: input.received, note,
  };
...
function parseProfile(input) {
  ...
  if (!Number.isInteger(input.branch) || input.branch < 0 || input.branch >= BRANCH_COUNT) return { error: 'Choose a valid branch.' };
  if (!['cod', 'upi'].includes(input.payment)) return { error: 'Choose a valid payment preference.' };
  return { value: { name, phone, house, area, landmark, branch: input.branch, payment: input.payment } };
}

const escapeRegex = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

module.exports = {
  BRANCH_COUNT, MAX_DELIVERY_PAISE,
  parseBill, priceBill, parseDeliveryCharge, parseProfile, normalizeIndianMobile, escapeRegex, text,
};