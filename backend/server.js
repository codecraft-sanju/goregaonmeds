...
// 🟢 NEW: Direct WhatsApp Integration Imports
...
/* ================================================================== */
/*  3. DIRECT WHATSAPP INTEGRATION (NATIVE BAILEYS)                   */
/* ================================================================== */
...
// Throttling logic to avoid spam bans
function waThrottleAllows(phoneRaw) {
  const now = Date.now();
  if (waSendLog.size > 5000) waSendLog.clear(); // cleanup

  const recent = (waSendLog.get(phoneRaw) || []).filter(at => now - at < WA_THROTTLE_WINDOW_MS);
...
// 🟢 NEW: MongoDB Auth Adapter for Baileys
// This saves WhatsApp session inside your DB so it survives Render restarts!
async function useMongoDBAuthState() {
  ...
  const readData = async (id) => {
    try {
      const doc = await WaAuth.findOne({ _id: id });
      if (doc) return JSON.parse(doc.data, BufferJSON.reviver);
      return null;
    } catch (error) { return null; }
  };
  const removeData = async (id) => {
    await WaAuth.deleteOne({ _id: id });
  };
...
// 🟢 NEW: Start WhatsApp Socket locally in this file
async function startWhatsApp() {
  ...
  try {
    ...
    waSocket = makeWASocket({
      ...
      logger: pino({ level: 'silent' }), // Hides huge logs
      browser: ['Lotus Pharmacy Console', 'Chrome', '1.0.0'],
    });
    ...
    waSocket.ev.on('connection.update', async (update) => {
      ...
      if (qr) {
        // Convert raw string QR to Base64 image URL for React frontend
        waQr = await QRCode.toDataURL(qr, { errorCorrectionLevel: 'M', margin: 1, width: 360 });
        waState = 'qr';
      }
      ...
      if (connection === 'close') {
        ...
        if (shouldReconnect) {
          console.log('[whatsapp] Connection dropped, reconnecting in 5s...');
          setTimeout(startWhatsApp, 5000);
        } else {
          console.log('[whatsapp] Logged out. Clearing DB session.');
          await WaAuth.deleteMany({}); // Delete session from DB if user logs out from phone
        }
      }

      if (connection === 'open') {
        console.log('[whatsapp] Connection Opened & Ready!');
        waState = 'connected';
        waQr = null;
        waPhone = waSocket.user.id.split(':')[0];
        waName = waSocket.user.name || 'Lotus Pharmacy Admin';
      }
    });
...
// 🟢 NEW: Send message directly from Baileys Socket
async function sendWhatsAppText(phone, text, context) {
  if (!waSocket || waState !== 'connected') {
    console.warn(`[whatsapp] Cannot send msg (${context}). WhatsApp is not connected.`);
    return false;
  }
  ...
  try {
    const jid = `${digitsOnly(phone)}@s.whatsapp.net`;
    await waSocket.sendMessage(jid, { text: text.slice(0, WA_MAX_TEXT) });
    return true;
  } catch (error) {
    console.error('[whatsapp] Send failed', { context, phone: maskPhone(phone), message: error?.message });
    return false;
  }
}
...
function formatOrderLines(items) {
  return items.map((item, index) =>
      `${index + 1}. ${sanitizeForMessage(item.displayName, 80)} × ${item.qty} — ${formatMoney(item.lineTotal)}`,
  );
}
...
function customerOrderMessage(order, branch) {
  ...
  if (order.type === 'prescription') {
    lines.push('', 'We have received your prescription photo. Our pharmacist will review it and send you the itemised bill here before dispatch.');
  } else {
    lines.push('', 'Items:', ...formatOrderLines(order.items), '', `Estimated total: ${formatMoney(order.estimatedTotal)}`);
    if (order.estimatedSavings > 0) lines.push(`You save ${formatMoney(order.estimatedSavings)} against MRP.`);
    lines.push('', 'This is an estimate. The pharmacist confirms stock and the final bill before dispatch.');
  }

  lines.push('', `Delivery to: ${formatCustomerAddress(order.customer)}`, `Branch contact: +${digitsOnly(branch.phone)}`, '', 'Reply here if anything needs changing.');
  return lines.join('\n');
}

function branchOrderAlertMessage(order, branch) {
  const lines = [
    `🔔 New ${order.type === 'prescription' ? 'prescription request' : 'order'} — ${sanitizeForMessage(branch.shortName || branch.name, 40)}`,
    '',
    `Order number: ${order.orderNumber}`,
  ...
  if (order.items.length > 0) lines.push('', 'Items:', ...formatOrderLines(order.items));

  lines.push('', `Estimated total: ${formatMoney(order.estimatedTotal)}`, `Prescription attached: ${order.hasPrescription ? 'yes (open it in the console)' : 'no'}`, '', 'Confirm stock and the bill in the Lotus Pharmacy console.');
  return lines.join('\n');
}
...
function statusUpdateMessage(order, branch, note) {
  ...
  if (note) lines.push('', `Note from the pharmacy: ${sanitizeForMessage(note, 300)}`);
  if (order.status !== 'cancelled' && order.estimatedTotal > 0) lines.push('', `Order value: ${formatMoney(order.estimatedTotal)}`);
  if (branch?.phone) lines.push('', `Branch contact: +${digitsOnly(branch.phone)}`);
  return lines.join('\n');
}

async function dispatchOrderNotifications(order, branch, { includeBranchAlert = true } = {}) {
  try {
    const customerSent = await sendWhatsAppText(customerChatId(order.customer.phone), customerOrderMessage(order, branch), 'order-confirmation');
    
    if (includeBranchAlert) {
      await sendWhatsAppText(branchChatId(branch.phone), branchOrderAlertMessage(order, branch), 'branch-order-alert');
    }

    if (!customerSent) return false;

    const now = new Date();
    await Order.updateOne({ _id: order._id, status: 'pending_whatsapp' }, { $set: { customerNotifiedAt: now }, $inc: { __v: 1 } });
    return true;
  } catch (error) {
    console.error('[whatsapp] Order notification failed', { orderNumber: order.orderNumber, message: error?.message });
    return false;
  }
}
...
async function dispatchStatusNotification(order, branch, note) {
  try {
    ...
    return await sendWhatsAppText(customerChatId(order.customer.phone), statusUpdateMessage(order, branch, note), `status-${order.status}`);
  } catch (error) {
    return false;
  }
}
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (!ACCEPTED_MIME.has(file.mimetype)) return cb(new ApiError(415, 'Upload a JPG, PNG, WEBP or HEIC image.'));
    cb(null, true);
  },
});
...
function uploadPublicProduct(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { folder: PRODUCT_FOLDER, resource_type: 'image', type: 'upload', overwrite: false, transformation: [{ width: 600, height: 600, crop: 'fill', gravity: 'auto' }, { quality: 'auto', fetch_format: 'auto' }] },
...
function uploadPrivatePrescription(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { folder: PRESCRIPTION_FOLDER, resource_type: 'image', type: 'authenticated', overwrite: false, transformation: [{ width: 1600, crop: 'limit' }, { quality: 'auto', fetch_format: 'auto' }] },
      (error, result) => (error ? reject(error) : resolve(result)),
    );
    stream.end(buffer);
  });
...
async function destroyCloudinaryAsset(publicId, type = 'upload') {
  ...
  try { await cloudinary.uploader.destroy(publicId, { resource_type: 'image', type, invalidate: true }); } catch (error) {}
}
...
function makePrescriptionUploadToken(publicId) {
  return jwt.sign({ purpose: 'prescription-upload', publicId }, config.jwtSecret, { expiresIn: '20m' });
}
...
function signedPrescriptionUrl(publicId) {
  return cloudinary.url(publicId, { resource_type: 'image', type: 'authenticated', secure: true, sign_url: true, expires_at: Math.floor(Date.now() / 1000) + 10 * 60 });
}
...
const medicineInput = z
  ...
  .refine((data) => data.mrp >= data.price, { message: 'MRP must be at least the selling price.', path: ['mrp'] });
...
const orderInput = z.object({
  ...
  items: z.array(z.object({ medicineId: z.string().min(1), buyType: z.enum(['full', 'loose']), qty: z.number().int().min(1).max(50) })).max(60).optional().default([]),
  ...
  try {
    for (const item of order.items) {
      ...
      const result = await Medicine.updateOne(
        { _id: item.medicine, inventory: { $elemMatch: { branch: order.branch, isAvailable: true, stockUnits: { $gte: units } } } },
        { $inc: { 'inventory.$.stockUnits': -units } },
      );
      if (result.modifiedCount !== 1) throw new ApiError(409, `${item.name} is no longer available in the required quantity.`);
      committed.push({ medicine: item.medicine, units });
    }
    order.stockCommitted = true;
  } catch (error) {
    for (const entry of committed) {
      await Medicine.updateOne({ _id: entry.medicine, 'inventory.branch': order.branch }, { $inc: { 'inventory.$.stockUnits': entry.units } }).catch(() => {});
    }
    throw error;
  }
}
...
async function releaseInventoryForOrder(order) {
  ...
  for (const item of order.items) {
    const units = item.stockUnitsPerQty * item.qty;
    await Medicine.updateOne({ _id: item.medicine, 'inventory.branch': order.branch }, { $inc: { 'inventory.$.stockUnits': units } });
  }
...
const login = asyncHandler(async (req, res) => {
  ...
  if (!user || !user.isActive || !(await user.comparePassword(parsed.password))) throw new ApiError(401, 'Email or password is incorrect.');
...
const updateProfile = asyncHandler(async (req, res) => {
  ...
  if (parsed.defaultAddress) req.user.addresses = [{ label: 'Home', ...parsed.defaultAddress }];
  await req.user.save();
...
const changePassword = asyncHandler(async (req, res) => {
  ...
  if (!user || !(await user.comparePassword(parsed.currentPassword))) throw new ApiError(401, 'Your current password is incorrect.');
...
function normalizeMedicineInventory(rows) {
  const seen = new Set();
  return rows.map((row) => {
    if (!Types.ObjectId.isValid(row.branchId)) throw new ApiError(422, 'One inventory branch id is not valid.');
    if (seen.has(row.branchId)) throw new ApiError(422, 'Each branch can appear only once in medicine inventory.');
    seen.add(row.branchId);
    return { branch: row.branchId, stockUnits: row.stockUnits, lowStockAt: row.lowStockAt, isAvailable: row.isAvailable };
  });
}
...
const listMedicines = asyncHandler(async (req, res) => {
  ...
  if (branchId && Types.ObjectId.isValid(branchId) && !isAdmin) {
    filter.inventory = { $elemMatch: { branch: branchId, isAvailable: true, stockUnits: { $gt: 0 } } };
  }
  if (search) {
...
const updateMedicine = asyncHandler(async (req, res) => {
  ...
  await existing.save();
  if (previousPublicId && previousPublicId !== existing.imagePublicId) await destroyCloudinaryAsset(previousPublicId, 'upload');
...
async function uniqueBranchSlug(name, ignoreId) {
  const base = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'branch';
  let slug = base;
  let suffix = 2;
...
const updateBranch = asyncHandler(async (req, res) => {
  ...
  const parsed = branchInput.parse(req.body);
  parsed.shortName = parsed.shortName || parsed.name.split(',')[0].trim().slice(0, 30);
  if (parsed.name !== branch.name) branch.slug = await uniqueBranchSlug(parsed.name, branch._id);
...
const createOrder = asyncHandler(async (req, res) => {
  ...
  if (!IDEMPOTENCY_RE.test(idempotencyKey)) throw new ApiError(400, 'A valid idempotency key is required.');

  const existingOrder = await Order.findOne({ idempotencyKey }).populate('branch', 'name shortName phone');
  if (existingOrder) return res.status(200).json({ order: orderPublicSummary(existingOrder, existingOrder.branch), duplicate: true });
  ...
  if (parsed.prescriptionToken && !prescriptionPublicId) throw new ApiError(422, 'The prescription upload expired. Upload it again.');
  if (parsed.type === 'prescription' && !prescriptionPublicId) throw new ApiError(422, 'Attach the prescription photo.', { prescription: 'Attach the prescription photo.' });
  ...
  if (parsed.type === 'cart') {
    ...
    const medicines = await Medicine.find({ _id: { $in: uniqueStrings(ids) }, isActive: true }).lean();
    ...
    items = parsed.items.map((raw) => {
      ...
      if (raw.buyType === 'loose' && !medicine.isDivisible) throw new ApiError(409, `${medicine.name} can no longer be bought loose.`);
      if (medicine.requiresPrescription && !prescriptionPublicId) throw new ApiError(422, `${medicine.name} requires a prescription.`);

      const stock = inventoryEntryFor(medicine, branch._id);
      const unitsNeeded = requiredStockUnits(medicine, raw.buyType, raw.qty);
      if (!stock || !stock.isAvailable || stock.stockUnits < unitsNeeded) throw new ApiError(409, `${medicine.name} does not have enough stock.`);

      const divisor = medicine.packSize > 0 ? medicine.packSize : 1;
      const unitPrice = raw.buyType === 'loose' ? round2(medicine.price / divisor) : medicine.price;
      const unitMrp = raw.buyType === 'loose' ? round2(medicine.mrp / divisor) : medicine.mrp;
      ...
      return {
        ...
        name: medicine.name,
        displayName: raw.buyType === 'loose' ? `${medicine.name} — 1 ${medicine.unitType.toLowerCase()}` : `${medicine.name} — ${medicine.packType.toLowerCase()} of ${medicine.packSize}`,
        buyType: raw.buyType,
        qty: raw.qty,
  ...
  if (req.user) {
    req.user.addresses = [{ label: 'Home', houseNo: parsed.customer.houseNo, area: parsed.customer.area, landmark: parsed.customer.landmark }];
  ...
  dispatchOrderNotifications(order, branch).catch(() => {});
});

const markWhatsAppOpened = asyncHandler(async (req, res) => {
  await Order.updateOne({ _id: req.params.id }, { $set: { whatsappOpenedAt: new Date() } });
  res.json({ message: 'WhatsApp handoff recorded.' });
});

const listMyOrders = asyncHandler(async (req, res) => {
  const orders = await Order.find({ user: req.user._id }).sort({ createdAt: -1 }).limit(50).populate('branch', 'name shortName phone address').lean();
  res.json({ orders });
});
...
const listAllOrders = asyncHandler(async (req, res) => {
  ...
  const [orders, total] = await Promise.all([
    Order.find(filter).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).populate('branch', 'name shortName phone address').populate('user', 'name email').lean(),
...
const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  if (!allowed.includes(parsed.status)) throw new ApiError(409, `Order cannot move from ${order.status} to ${parsed.status}.`);
  ...
  order.statusHistory.push({ status: parsed.status, at: new Date(), by: req.user._id, note: parsed.note });

  try {
    await order.save();
  } catch (error) {
    if (committedNow) await releaseInventoryForOrder(order).catch(()=>{});
    if (releasedNow) {
      order.stockCommitted = false;
      await commitInventoryForOrder(order).catch(()=>{});
    }
    throw error;
  }
  ...
  await order.populate('branch', 'name shortName phone address');
  res.json({ order });
  dispatchStatusNotification(order, order.branch, parsed.note).catch(()=>{});
});
...
const resendOrderNotification = asyncHandler(async (req, res) => {
  ...
  const sent = order.status === 'pending_whatsapp' ? await dispatchOrderNotifications(order, order.branch, { includeBranchAlert: false }) : await dispatchStatusNotification(order, order.branch, '');
  if (!sent) throw new ApiError(502, 'The message was not delivered. Check the WhatsApp connection.');

  res.json({ order, message: 'WhatsApp message sent.' });
});

const getPrescriptionForOrder = asyncHandler(async (req, res) => {
  const order = await Order.findById(req.params.id).select('user prescriptionPublicId hasPrescription');
  if (!order || !order.hasPrescription) throw new ApiError(404, 'No prescription found.');
...
const whatsappConnect = asyncHandler(async (req, res) => {
  if (waState === 'disconnected' || !waSocket) {
    await startWhatsApp();
  }
  const pendingCount = await countUnnotifiedOrders();
  res.json({
    session: { configured: true, reachable: true, state: waState, phone: waPhone, name: waName, qr: waQr, message: '', checkedAt: new Date().toISOString() },
    pendingCount
  });
});
...
const whatsappLogout = asyncHandler(async (req, res) => {
  ...
  await WaAuth.deleteMany({}); // Purge memory
  waState = 'disconnected';
  waQr = null;
  waPhone = '';
  waName = '';
  res.json({ session: { configured: true, reachable: true, state: waState, phone: waPhone, name: waName, qr: waQr, message: '', checkedAt: new Date().toISOString() }, pendingCount: await countUnnotifiedOrders() });
});

const whatsappTest = asyncHandler(async (req, res) => {
  const { phone } = whatsappTestInput.parse(req.body);
  const text = ['Test message from Lotus Pharmacy.', `Sent by ${req.user.name}.`, 'Your direct WhatsApp server is working!'].join('\n');
  const sent = await sendWhatsAppText(customerChatId(phone), text, 'admin-test');
  if (!sent) throw new ApiError(502, 'Message not delivered. Ensure WhatsApp is connected.');
  res.json({ message: 'Test message sent.' });
});

const whatsappRetryPending = asyncHandler(async (_req, res) => {
  const orders = await Order.find({ status: 'pending_whatsapp', customerNotifiedAt: null, createdAt: { $gte: new Date(Date.now() - WA_RETRY_WINDOW_MS) } }).sort({ createdAt: 1 }).limit(WA_RETRY_BATCH).populate('branch');
  let sent = 0;
  for (const order of orders) {
    if (!order.branch) continue;
    if (await dispatchOrderNotifications(order, order.branch, { includeBranchAlert: false })) sent += 1;
  }
  res.json({ attempted: orders.length, sent, pendingCount: await countUnnotifiedOrders() });
});
...
const adminStats = asyncHandler(async (_req, res) => {
  ...
  const [ ordersToday, ordersYesterday, waitingWhatsApp, openOrders, medicineCount, bookedTodayAgg, bookedYesterdayAgg, dailyAgg, branchAgg ] = await Promise.all([
    Order.countDocuments({ createdAt: { $gte: today } }),
    Order.countDocuments({ createdAt: { $gte: yesterday, $lt: today } }),
    Order.countDocuments({ status: 'pending_whatsapp' }),
    Order.countDocuments({ status: { $in: ['pending_whatsapp', 'placed', 'confirmed', 'packed', 'out_for_delivery'] } }),
    Medicine.countDocuments({ isActive: true }),
    Order.aggregate([{ $match: { createdAt: { $gte: today }, status: { $in: bookedStatuses } } }, { $group: { _id: null, total: { $sum: '$estimatedTotal' } } }]),
    Order.aggregate([{ $match: { createdAt: { $gte: yesterday, $lt: today }, status: { $in: bookedStatuses } } }, { $group: { _id: null, total: { $sum: '$estimatedTotal' } } }]),
    Order.aggregate([{ $match: { createdAt: { $gte: weekStart } } }, { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$createdAt', timezone: BUSINESS_TIMEZONE } }, orders: { $sum: 1 }, bookedValue: { $sum: { $cond: [{ $in: ['$status', bookedStatuses] }, '$estimatedTotal', 0] } } } }]),
    Order.aggregate([{ $match: { createdAt: { $gte: monthStart } } }, { $group: { _id: '$branch', orders: { $sum: 1 } } }, { $sort: { orders: -1 } }, { $limit: 8 }]),
  ]);
  ...
  const branchDocs = await Branch.find({ _id: { $in: branchAgg.map((row) => row._id) } }).select('name shortName').lean();
  const branchNames = new Map(branchDocs.map((branch) => [branch._id.toString(), branch]));
  const busiest = branchAgg[0]?.orders || 0;
  const branchPerformance = branchAgg.filter((row) => branchNames.has(String(row._id))).map((row) => {
    const branch = branchNames.get(String(row._id));
    return { id: String(row._id), name: branch.shortName || branch.name, orders: row.orders, pct: busiest > 0 ? Math.round((row.orders / busiest) * 100) : 0 };
  });

  res.json({ ordersToday, ordersDelta: percentChange(ordersToday, ordersYesterday), waitingWhatsApp, openOrders, bookedToday: round2(bookedTodayAgg[0]?.total || 0), bookedDelta: percentChange(round2(bookedTodayAgg[0]?.total || 0), round2(bookedYesterdayAgg[0]?.total || 0)), medicineCount, ordersSeries, revenueSeries, branchPerformance });
});
...
const uploadPrescription = asyncHandler(async (req, res) => {
  ...
  res.status(201).json({ uploadToken: makePrescriptionUploadToken(result.public_id) });
});
...
const deleteProductUpload = asyncHandler(async (req, res) => {
  ...
  if (!publicId || !publicId.startsWith(`${PRODUCT_FOLDER}/`)) throw new ApiError(403, 'Asset is invalid.');
...
uploadRouter.post('/prescription', uploadLimiter, requireCloudinary, upload.single('image'), uploadPrescription);
uploadRouter.post('/prescription/revert', uploadLimiter, requireCloudinary, revertPrescriptionUpload);
...
adminRouter.post('/whatsapp/connect', protect, adminOnly, waActionLimiter, noStore, whatsappConnect);
adminRouter.post('/whatsapp/logout', protect, adminOnly, waActionLimiter, noStore, whatsappLogout);
adminRouter.post('/whatsapp/test', protect, adminOnly, waActionLimiter, whatsappTest);
adminRouter.post('/whatsapp/retry-pending', protect, adminOnly, waActionLimiter, whatsappRetryPending);
...
app.use((err, req, res, _next) => {
  ...
  if (err instanceof multer.MulterError) {
    ...
    details = Object.fromEntries((err.issues || err.errors || []).map((issue) => [issue.path.join('.'), issue.message]));
  } else if (err.name === 'ValidationError' && err.errors) {
    statusCode = 422;
    message = 'Some fields need attention.';
    details = Object.fromEntries(Object.entries(err.errors).map(([field, error]) => [field, error.message]));
    ...
    statusCode = 409;
    message = err.keyPattern?.idempotencyKey ? 'This request was already processed.' : 'That record already exists.';
  }

  if (statusCode >= 500) {
    console.error('[error]', { requestId: req.requestId, method: req.method, path: req.originalUrl, message: err.message });
    if (isProd && statusCode === 500) message = 'Something went wrong on our side.';
  }

  res.status(statusCode).json({ message, requestId: req.requestId, ...(details ? { details } : {}) });
});
...
async function seedAdmin() {
  ...
  if (!existing) {
    await User.create({ name: 'Pharmacy Admin', email, phone: config.seedAdmin.phone || '9999999999', password: config.seedAdmin.password, role: 'admin' });
    console.log(`[admin] Created seed admin: ${email}`);
    return;
  }
  existing.role = 'admin';
...
async function start() {
  ...
  await mongoose.connect(config.mongoUri, { serverSelectionTimeoutMS: 10000, autoIndex: !isProd });
  console.log('[db] Connected.');

  await seedAdmin();
  
  // 🟢 Start WhatsApp internally when DB is connected
  startWhatsApp();
...
if (require.main === module) {
  start().catch((error) => { console.error('[fatal] Startup failed:', error); process.exit(1); });
}
