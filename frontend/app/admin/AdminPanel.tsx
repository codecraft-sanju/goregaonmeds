...
import { AdminWelcome, OfferProduct, GLUCOONE_MRP_PAISE, isGlucoOne } from '../components/WelcomeExperience';
import './admin.css';
...
export default function AdminPanel() {
  ...
  async function prepareShare() {
    ...
    await task('Preparing WhatsApp',async()=> {
      ...
      const giftLine = finalBill.freeGift ? `🎁 Free gift: ${freeGiftLabel(finalBill.freeGift.name)}${isGlucoOne(finalBill.freeGift.name) ? ` (MRP ${money(GLUCOONE_MRP_PAISE)} · Charged ₹0)` : ''}\n` : '';
  ...
  return <div className="ga ga-app">
    ...
    <main className="ga-main"><AdminWelcome disabled={!!busy || dialog !== null}/><div className="ga-page-heading"><div><span className="ga-kicker">{shortDate(bill.createdAt)} · GOREGAON EAST</span><h1>{headingTitle}</h1><p>{headingText}</p></div><button className="ga-btn ga-primary" onClick={()=>setDialog('new')} disabled={!!busy}><Plus size={18}/> New bill</button></div>
    ...
    {view==='billing' ? <>
      ...
      {firstOrderOffer && <OfferProduct offer={firstOrderOffer} compact/>}
